...
# --- PERBAIKAN DI SINI ---
@export var daftar_animasi : Array[String] = [] # Isi dengan nama animasi, misal: "api", "lubang", "npc"
@onready var anim_node = $npc # Pastikan ini AnimatedSprite2D
...
func _ready():
	# Memilih animasi secara acak dari SpriteFrames
	pilih_animasi_acak()
...
func pilih_animasi_acak():
	if daftar_animasi.size() > 0 and anim_node:
		var indeks = randi() % daftar_animasi.size()
		var nama_terpilih = daftar_animasi[indeks]
		
		# PENTING: Gunakan .play(), bukan .texture
		anim_node.play(nama_terpilih) 
	else:
		print("Peringatan: Daftar animasi kosong atau node tidak ditemukan!")
